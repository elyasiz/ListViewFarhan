...
OnClickListener {
private String torestoXmlAddress = "http://toresto.com/restaurant.xml";
private String torestoJsonAddress = "http://toresto.com/restaurant.json";
...
URL url = new URL(torestoXmlAddress);
 ...
 final Result restaurant = serializer.read(Result.class, is);
 runOnUiThread(new Runnable() {
 @Override
 public void run() {
 Toast toast = Toast.makeText(MainActivity.this, "Ada " +restaurant.getData().size() + " data",Toast.LENGTH_LONG);
 toast.setGravity(Gravity.TOP | Gravity.RIGHT, 0, 0);
 toast.show();
 items.clear();
 Map<String, String> map; 
 for (int i = 0; i <restaurant.getData().size(); i++) {
 map = new HashMap<String,String>();
 map.put(ID,Integer.toString(restaurant.getData().get(i).getId
 ()));
 map.put(NAMA,restaurant.getData().get(i).getName());
 map.put(ALAMAT,restaurant.getData().get(i).getAddress());
 map.put(PHONE,restaurant.getData().get(i).getPhone().getExt()+ " - " +
 restaurant.getData().get(i).getPhone().getText());
 items.add(map);
 }
 String[] from = new String[] { ID, NAMA, ALAMAT, PHONE };
 int[] to = new int[] { R.id.textView1,R.id.textView2,R.id.textView3, R.id.textView4 };
 SimpleAdapter adapter = new
 SimpleAdapter(MainActivity.this, items,R.layout.custom_list_4,from, to);
...
String response = null; HttpClient
httpclient = null;
try {
HttpGet httpget = new HttpGet(url); httpclient =new DefaultHttpClient(); 
...
public GResult parseGson(String url) throws JSONException {
	...
	Gson gson = new Gson();
	GResult gResult = gson.fromJson(response,GResult.class);
	...
	dialog.cancel(); 
	if (result != null) {
	Toast.makeText(MainActivity.this,"Ada "+result.getResult().getCount() + "data",Toast.LENGTH_SHORT).show();
	items.clear();
	Map<String, String> map; for
	(int i = 0; i <
	result.getResult().getData().size(); i++) {
	map = new HashMap<String, String>();
	map.put(ID,Integer.toString(result.getResult().getData().get(
	i).getId()));
	map.put(NAMA,result.getResult().getData().get(i).getName());
	map.put(ALAMAT,result.getResult().getData().get(i).getAddress());
			items.add(map);
			}
			String[] from = new String[] { ID, NAMA,ALAMAT };
			int[] to = new int[] { R.id.textView1,R.id.textView2, R.id.textView3};
			SimpleAdapter adapter = new
			SimpleAdapter(MainActivity.this,items, R.layout.custom_list_3,from, to);
			listView.setAdapter(adapter);
			} else {
			// munculkan pesan error
			Toast.makeText(MainActivity.this,"Tidak dapat membaca data",Toast.LENGTH_SHORT).show();
			 }
			 }
			 }
			}